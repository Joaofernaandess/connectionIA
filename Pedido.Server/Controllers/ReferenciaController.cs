...
public static class ReferenciaController
{
    ...
    {
        #region [ post ]
        app.MapPost("v1/referencias/", async (ReferenciaService service, ReferenciaPostRequest referencia) =>
        {
            var result = await service.Cadastrar(referencia);
       ...
       .RequireAuthorization();
        ...
        #region [ put ]
        app.MapPut("v1/referencias/{referenciaId:guid}", async (ReferenciaService service, Guid referenciaId, ReferenciaPutRequest referencia) =>
        {
            await service.Atualizar(referenciaId, referencia);
