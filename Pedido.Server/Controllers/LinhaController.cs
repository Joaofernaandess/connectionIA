...
public static class LinhaController
{
    ...
    {
        #region [ post ]
        app.MapPost("v1/linhas/", async (LinhaService service, LinhaPostRequest linha) =>
        {
            Guid linhaId = await service.Cadastrar(linha);
       ...
       .RequireAuthorization();
        ...
        #region [ put ]
        app.MapPut("v1/linhas/{linhaId:guid}", async (LinhaService service, Guid linhaId, LinhaPutRequest linha) =>
        {
            await service.Atualizar(linhaId, linha);
