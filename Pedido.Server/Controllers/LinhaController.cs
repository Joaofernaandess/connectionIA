...
public static class LinhaController
{
    ...
    {
        #region [ post ]
        app.MapPost("v1/linhas/", async (HttpContext context, LinhaService service, LinhaPostRequest linha) =>
        {
            ...
            Guid linhaId = await service.Cadastrar(linha, usuarioId, nomeUsuario);
        ...
        #region [ put ]
        app.MapPut("v1/linhas/{linhaId:guid}", async (HttpContext context, LinhaService service, Guid linhaId, LinhaPutRequest linha) =>
        {
            var usuarioIdClaim = context.User.FindFirstValue(ClaimTypes.NameIdentifier);
            var nomeUsuario = context.User.FindFirstValue(ClaimTypes.Name) ?? "Usuário não identificado";
            ...
            await service.Atualizar(linhaId, linha, usuarioId, nomeUsuario);
